...
impl<'a> RawAnswer<'a> {
    ...
    pub fn init<F: FnMut(&RawHeader<'a>) -> Option<()>>(
        ...
        for _ in 0..ancount {
            self.answer.push(RawRecord::new(&mut self.reader, &mut map)?);
        }
        
        for _ in 0..nscount {
            self.authority.push(RawRecord::new(&mut self.reader, &mut map)?);
        }
        
        for _ in 0..arcount {
            self.additional.push(RawRecord::new(&mut self.reader, &mut map)?);
        }
...
mod test {
    ...
    fn test() {
        let mut raw = RawAnswer::new(&[
            ...
            0x00, 0x01, 0x00, 0x00, 0x00, 0x1e, 0x00, 0x10, 0x24, 0x08, 0x87, 0x11, 0x00, 0x10,
            0x10, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x25][..]).unwrap();
